...
const FormPhoneInput = React.forwardRef<
  ...
  FormPhoneInputProps
>(({ className, control, name, rules, ...props }, ref) => {
  const {
...
const PhoneInputComponent = React.forwardRef<HTMLInputElement, InputProps>(
  ({ className, ...props }, ref) => (
    <Input
    ...
    <Popover>
      <PopoverTrigger asChild>
        <Button
          type="button"
          variant="outline"
          className={cn("flex gap-1 h-9 rounded-e-none rounded-s-lg px-3")}
          disabled={disabled}
        >
